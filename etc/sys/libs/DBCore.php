...
class DBCore
{
		...
		try {
			...
			switch ($type)
			{
				...
				case 'sqlite':
					...
					break;
				case 'odbc':
					if($this->db_type) $this->database_type = $this->db_type;
					$dsn = 'odbc:'.$this->odbc_dsn;
					break;
			}
			$this->pdo = new PDO(
					$dsn,
    ...
    public function orderBy($column){
        if(!isset($this->whereRules['ORDER'])){
            if(is_array($column)) $this->whereRules['ORDER'] = $column;
            else $this->whereRules['ORDER'] = array($column);
        }else{
            $this->whereRules['ORDER'][] = $column;
        }
        return $this;
    }
    ...
    public function limit($limit){
        if(!is_array($limit)){
			if(func_num_args() == 1){
				...
				$limit = array($limit,func_get_arg(1));
			}
        }
        $this->whereRules['LIMIT'] = $limit;
        return $this;
    }
    public function where($where){
        if(is_string($where)){
            $where = array($where => func_get_arg(1));
        }
        ...
        if($this->whereRules){
            if(!empty($single_condition)){
                $this->whereRules['AND'] = array_merge($this->whereRules['AND'],$where);
            }else{
                $this->whereRules = array_merge($this->whereRules,$where);
            }
            return $this;
        }
        if(!empty($single_condition)){
            $this->whereRules = array(
                'AND' => $where
            );
            return $this;
        }
        $this->whereRules = $where;
        return $this;
    }
...
//            $fields = '`'.implode('`,`',explode(',',$fields)).'`';
	...
	public function query($query)
	{
        ...
        $this->fields = '*';
        $queryItem = array('sql' =>$query,'time' => 0);
        try{
            $startTime = microtime(true);
            $data = $this->pdo->query($query);
            $endTime = microtime(true);
            $spentTime = ($endTime - $startTime);
            $this->runTimes += $spentTime;
            $queryItem['time'] = round($spentTime,3).' sec';
            $this->queryItems[] = $queryItem;
            return $data;
        }catch (Exception $e){
			$message = $e->getMessage();
			if(strtolower($this->charset) == 'gbk' && RouterCore::getInstance()->getConf('charset') != 'gbk'){
				$message = gbk2utf8($message);
			}
	...
	protected function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
	{
		...
		)
		{
			...
			{
				preg_match('/(\[(\<|\>|\>\<|\<\>)\])?([a-zA-Z0-9_\-]*)(#[a-zA-Z]*)?/', $sub_table, $match);
                $asName = '';
				if ($match[2] != '' && $match[3] != '')
				{
                    $rightTable = '';
                    if($match[4]){
                        $rightTable = substr($match[4],1);
                        $asName = $rightTable;
                    }else{
                        $rightTable = $this->db_prefix.$match[3];
                    }
					...
					if (is_array($relation))
					{
						...
						{
                            $leftColumn =  key($relation);
                            $rightColumn =  current($relation);
                            if(strpos($leftColumn,'.') == -1){
                                $leftColumn = $table.'.'.$leftColumn.'';
                            }


                            if(strpos($rightColumn,'.') == -1){
                                $rightColumn = $rightTable.'.'.$rightColumn;
                            }
//							$relation = 'ON ' .$table . '."' . key($relation) . '" = "' . $match[3] . '"."' . current($relation) . '"';
							$relation = 'ON ' .$leftColumn . ' = ' . $rightColumn;
						}
					}
					$table_join[] = $join_array[ $match[2] ] . ' JOIN ' . $this->db_prefix.$match[3] . ' '.$asName.' ' . $relation;
				}
			}
	...
	public function select($table = null, $join = null, $columns = null, $where = null)
	{
        ...
        if($columns) $this->fields = $columns;
        if($where) $this->whereRules = $where;

		$query = $this->query($this->select_context($this->tableName, $this->join, $this->fields, $this->whereRules));
	...
	public function update($table, $data, $where = null)
	{
		...
		return $this->exec('UPDATE `' . $this->db_prefix.$table . '` SET ' . implode(', ', $fields) . $this->where_clause($where));
	}

	public function delete($table, $where)
	{
		return $this->exec('DELETE FROM `' . $this->db_prefix.$table . '`' . $this->where_clause($where));
	}
	...
	public function get($table = null,$join = null, $columns = null, $where = null)
	{
		if (!isset($where))
		{
            if (!isset($this->whereRules))
            {
                $this->whereRules = array();
            }
			$where = $this->whereRules;
		}
		$where['LIMIT'] = 1;
    ...
    public function countBySql($sql){
        return 0 + ($this->query($sql)->fetchColumn());
    }
