...
class Logger{
    ...
    private function Logger($tagName){
        if($tagName){
...
//        RouterCore::getInstance()->addInterceptor('ended',array($this,'showLogList'));
    ...
    private function log($message,$level){
        ...
        $line = sprintf('%s [%s] (%s):<span style="color:%s">%s</span>     ',
            $time,$this->tagName.'('.$line.')',$this->getLevelText($level),$color,$message);
        if($level >= $this->level){
    ...
    public function showLogList(){
        if(!RouterCore::getInstance()->getConf('show_log')) return;
        echo '<style>
