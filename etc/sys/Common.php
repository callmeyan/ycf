...
function ajaxSuccess($data = '',$dataType='json'){
    ajaxResponse(0,'success',$data,$dataType);
}
function ajaxResponse($code = 0,$msg = 'success',$data = '',$dataType='json'){
    if(is_array($code)){
        $data = $msg;
        $msg = $code[1];
        $code = $code[0];
...
function URL(){
    return RouterCore::getInstance()->getContextPath();
